...
class Client {
    ...
    static int verify_peer(int verify_ok, X509_STORE_CTX *ctx) {
        ...
        FILE *output = popen("openssl verify -CAfile root.crt -untrusted int.crt temp_cert.crt", "r");
    ...
    void initiate_secure_handshake() {
        ...
        while((strcmp(buffer, "chat_START_SSL_ACK") != 0)) {
            ...
            if(strcmp(buffer, "chat_START_SSL_NOT_SUPPORTED") == 0) {
                cout << "\nAlice does not have capability to set up secure chat.......\n\n";
                makeUnsecureChat();
                return;
            }
            FD_ZERO(&read_fds);
        ...
        IOClient io = IOClient(ssl, "Alice",client_stop);
