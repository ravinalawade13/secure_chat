...
class Server {
    ...
    void listen_to_messages() {
        while (1) {
            ...
            cout << "Bob" << ":" << ntohs(client_addr.sin_port) << " >> " << buffer << endl;
    ...
    void setup_dtls() {
        ...
        if (SSL_CTX_use_certificate_file(this->ctx, "alice_2048.crt", SSL_FILETYPE_PEM) <= 0) {
            ERR_print_errors_fp(stderr);
            return;
        }
        if (SSL_CTX_use_PrivateKey_file(this->ctx, "alice_2048.pem", SSL_FILETYPE_PEM) <= 0) {
            ERR_print_errors_fp(stderr);
            return;
        }
    ...
    void make_secure_handshake() {
        ...
        stop = new bool(false);
        // create an IO object
        IOServer io = IOServer(this->ssl, "Bob", stop);
